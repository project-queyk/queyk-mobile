...
export default function EvacuationPlan() {
    ...
    (async () => {
      ...
      try {
        const res = await ensureWatcherStarted?.({
          ...
          attempts: isOffline ? 3 : 2,
          waitForAltitudeMs: isOffline ? 6000 : 4000,
        });
        clearAwaitingTimeout();
        if (!res?.success) {
  ...
  useEffect(() => {
    ...
    if (altitude <= 122.0) {
      selectedFloorValue = "ground";
    } else if (altitude > 122.0 && altitude <= 125.5) {
      selectedFloorValue = "first";
    } else if (altitude > 125.5 && altitude <= 127.8) {
      selectedFloorValue = "second";
    } else if (altitude > 127.8 && altitude <= 129.5) {
      selectedFloorValue = "third";
    } else if (altitude > 129.5) {
      selectedFloorValue = "fourth";
    }
            ...
            if (current.status === "granted") {
              ...
              try {
                const res = await ensureWatcherStarted?.({
                  promptIfNeeded: false,
                  attempts: isOffline ? 3 : 2,
                  waitForAltitudeMs: isOffline ? 8000 : 5000,
                });
                clearAwaitingTimeout();
                if (!res?.success) {
                  if (res?.permission && res.permission.status !== "granted") {
